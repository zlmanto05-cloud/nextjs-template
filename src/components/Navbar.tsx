...
export default function Navbar() {
  ...
  return (
    ...
    >
      {/* h-20 = 80px de alto (antes h-16 = 64px) */}
      <div className="max-w-7xl mx-auto px-6 md:px-10 flex items-center justify-between h-20">
        ...
        {/* Desktop links — texto más grande y bold */}
        <nav className="hidden md:flex items-center gap-8" aria-label="Navegación principal">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="text-gray-700 hover:text-[#3b55f5] text-[15px] font-semibold transition-colors"
            >
              {l.label}
            </a>
          ))}
        ...
        {/* Desktop WAB CTA — más grande */}
        <a
          href={WA_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="hidden md:inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#1ebe5d] text-white text-[15px] font-bold px-6 py-3 rounded-full transition-colors"
        ...
        <div className="flex md:hidden items-center gap-3">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 bg-[#25D366] hover:bg-[#1ebe5d] text-white text-[13px] font-bold px-4 py-2.5 rounded-full transition-colors"
          >
            <WaIcon />
            Cotizar
          </a>
          ...
          >
            {menuOpen ? (
              ...
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg>
            )}
          </button>
        </div>
        ...
        <nav className="md:hidden bg-white border-t border-gray-100 px-6 py-4 flex flex-col gap-1" aria-label="Menú móvil">
          {links.map((l) => (
            <a
              ...
              href={l.href}
              onClick={closeMenu}
              className="text-gray-700 hover:text-[#3b55f5] font-semibold py-3 border-b border-gray-50 text-base transition-colors"
            >
              {l.label}
            </a>
          ))}
          <a
            ...
            rel="noopener noreferrer"
            onClick={closeMenu}
            className="mt-3 flex items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-3.5 rounded-xl text-[15px] transition-colors"
          >
            <WaIcon />
